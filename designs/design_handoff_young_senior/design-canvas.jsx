...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  // Fit-to-width on first paint so the canvas isn't stuck at the top-left of a
  // huge world at scale 1. Runs once when content is large enough to need it.
  React.useLayoutEffect(() => {
    const vp = vpRef.current, w = worldRef.current;
    if (!vp || !w) return;
    const fit = () => {
      const vpW = vp.clientWidth, vpH = vp.clientHeight;
      const wW = w.scrollWidth, wH = w.scrollHeight;
      if (!vpW || !wW) return;
      const scale = Math.min(1, vpW / wW, vpH / wH);
      tf.current.scale = Math.max(minScale, scale);
      // center horizontally; pin to top so first section is visible
      tf.current.x = Math.max(0, (vpW - wW * tf.current.scale) / 2);
      tf.current.y = 0;
      apply();
    };
    // Wait a frame so children measure correctly.
